...
# Install a bunch of system dependencies needed for a lot of R packages
...
RUN echo 'install.packages(c(' >> install_packages.R && \
  ...
  echo '"mirt", "roxygen2", "rmarkdown", "nonnest2",'     >> install_packages.R && \
  echo '"markdown", "pkgdown", "usethis", "rcmdcheck",'   >> install_packages.R && \
  echo '"rversions", "urlchecker", "tinytex", "sirt"'     >> install_packages.R && \
